...
const FuzzyText: React.FC<FuzzyTextProps> = ({
  ...
  const glitchSetterRef = useRef<(v: boolean) => void>(() => {});
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      glitchSetterRef.current = (v: boolean) => { isGlitching = v; };
  ...
  useEffect(() => {
    if (!glitchOnEvent) return;
    let timeoutId: ReturnType<typeof setTimeout>;
    const handler = () => {
      glitchSetterRef.current(true);
      clearTimeout(timeoutId);
      timeoutId = setTimeout(() => glitchSetterRef.current(false), glitchDuration);
    };
    window.addEventListener(glitchOnEvent, handler);
    return () => {
      window.removeEventListener(glitchOnEvent, handler);
      clearTimeout(timeoutId);
    };
  }, [glitchOnEvent, glitchDuration]);
